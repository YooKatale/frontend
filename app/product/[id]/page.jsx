...
const Product = ({ params }) => {
  ...
  const [exhangeRate, setExchangeRate] = useState(1);
  ...
  // function to handle getting the exchange rate base 
  async function getExchangeRate(currency) {
    // extract the first 4 numbers phone number in the userinfo
    const phone = userInfo?.phone.slice(0, 4);
    const phoneSa = userInfo?.phone.slice(0, 3);
    const ratesBasedOnUGX = await axios.get(
      `https://v6.exchangerate-api.com/v6/1ca4ea04c689626a7c17b86a/latest/${currency}`)

    if (phone == "+254" || phoneSa == "+254") {
      const kenya = ratesBasedOnUGX.conversion_rates.KES;
      setExchangeRate(kenya);
    } else if (phone == "+250" || phoneSa == "+250") {
      const rwanda = ratesBasedOnUGX.conversion_rates.RWF;
      setExchangeRate(rwanda);
    } else if (phone == "+255" || phoneSa == "+255") {
      const tanzania = ratesBasedOnUGX.conversion_rates.TZS;
      setExchangeRate(tanzania);
    } else if (phone == "+234" || phoneSa == "+234") {
      const nigeria = ratesBasedOnUGX.conversion_rates.NGN;
      setExchangeRate(nigeria);
    } else if (phone == "+233" || phoneSa == "+233") {
      const ghana = ratesBasedOnUGX.conversion_rates.GHS;
      setExchangeRate(ghana);
    } else if (phone == "+27" || phoneSa == "+27") {
      const sa = ratesBasedOnUGX.conversion_rates.ZWD;
      setExchangeRate(zimbabwe);
    }
  }
        ...
        <Box>
          <Box padding={"0 2rem"}>
            ...
            <Box padding={{ base: "1rem 0", md: "1rem 0", xl: "1rem 2rem" }}>
              ...
              >
                <Box width={{ base: "100%", md: "90%", xl: "45%" }}>
                  <Box id="main-product-image" position={"relative"}>
                    ...
                    <Box padding={"1rem 0"}>
                      <Grid
                        gridTemplateColumns={`repeat(${parseInt(DisplayImages.length)
                    ...
                    >
                      ...
                      {isEditingPrice ? (
                        <input
                          type="number"
                          value={editablePrice}
                          onChange={(e) => setEditedPrice(e.target.value * exhangeRate)}
                          className="border"
                        />
                      ) : (
                        FormatCurr(ProductData?.price ? ProductData?.price * exhangeRate : 0)
                      )}
                      {ProductData?.type !== "bulk" && (
              ...
              {/* // signin / signup form */}
              <div
                className={`fixed top-[10%] lg:left-[30%] left-[5%] lg:right-[30%] right-[5%] bottom-[10%] z-[990] bg-light py-6 rounded-md shadow-md ${SignInStateModal
