...
/**
 * Get today's meals from the meal calendar for a specific meal type.
 * Returns both ready-to-eat and ready-to-cook options for email display.
 * @param {string} mealType - breakfast, lunch, or supper
 * @returns {Array<{meal: string, type: string, quantity: string, description?: string, image?: string}>}
 */
export function getTodaysMealsFromCalendar(mealType) {
  const today = getCurrentDay();
  const result = [];

  try {
    const readyToEat = getMealForDay(today, mealType, "middle", "ready-to-eat");
    if (readyToEat) {
      result.push({
        meal: readyToEat.meal,
        type: "ready-to-eat",
        quantity: readyToEat.quantity || "1 serving",
        description: readyToEat.description,
        image: readyToEat.image,
      });
    }
    const readyToCook = getMealForDay(today, mealType, "middle", "ready-to-cook");
    if (readyToCook) {
      result.push({
        meal: readyToCook.meal,
        type: "ready-to-cook",
        quantity: readyToCook.quantity || "1 serving",
        description: readyToCook.description,
        image: readyToCook.image,
      });
    }
  } catch (error) {
    console.error("Error getting meals from mealMenuConfig:", error);
  }

  if (result.length > 0) return result;
  // Fallback to legacy single meal
  return getTodaysMeals(mealType);
}
