...
async function triggerNotification(userInfo = null) {
  try {
    ...
    try {
      ...
      // Silently handle - don't log in production
      if (!response.ok && process.env.NODE_ENV === 'development') {
        const errorText = await response.text();
        console.warn("Notification trigger response:", response.status, errorText);
      }
    } catch (apiError) {
      // Silently handle - backend scheduler handles it
      if (process.env.NODE_ENV === 'development') {
        console.warn("Error calling notification endpoint:", apiError.message);
      }
    }
  } catch (error) {
