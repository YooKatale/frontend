...
export function startNotificationPolling(userInfo = null) {
  ...
  // Then poll every minute (60000 ms)
...
async function triggerNotification(userInfo = null) {
  try {
    // Get current time to determine meal type (optional - for personalized notifications)
    const now = new Date();
    ...
    const response = await fetch("/api/trigger-push-notification", {
      ...
      body: JSON.stringify({
        ...
        testMode: true, // Indicates this is a test/periodic notification
      }),
    });
