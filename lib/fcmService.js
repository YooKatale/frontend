...
import { getMessagingSafe, getToken, onMessage } from "@lib/firebase";
...
export async function saveFCMTokenToServer(token, userInfo) {
  try {
    ...
    // Always send both userId and email when available - backend can use either
    // Google sign-in users may have id instead of _id; email is reliable for all auth methods
    ...
    const response = await fetch(`${backendUrl}/admin/web_push`, {
      ...
      credentials: "omit", // No cookies needed; user identity in body
      body: JSON.stringify({
        token: token,
