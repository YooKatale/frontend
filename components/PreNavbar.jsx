...
/* ─── EID ICONS ──────────────────────────────────────────────────────────── */
const CrescentMoon = ({ s = 24 }) => (
  <svg width={s} height={s} viewBox="0 0 24 24" fill="none">
    <path
      d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"
      fill="#f0c060" stroke="#c9a227" strokeWidth="1.2"
    />
  </svg>
);

const StarFill = ({ s = 10, opacity = 1 }) => (
  <svg width={s} height={s} viewBox="0 0 24 24" fill={`rgba(240,192,96,${opacity})`}>
    <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
  </svg>
);

const LanternIcon = ({ s = 22 }) => (
  <svg width={s} height={s} viewBox="0 0 24 24" fill="none">
    <rect x="7" y="5" width="10" height="14" rx="3" fill="#c9a227" opacity="0.9"/>
    <rect x="9" y="7" width="6" height="10" rx="2" fill="#fff3c4" opacity="0.7"/>
    <line x1="12" y1="2" x2="12" y2="5" stroke="#c9a227" strokeWidth="1.5" strokeLinecap="round"/>
    <line x1="12" y1="19" x2="12" y2="22" stroke="#c9a227" strokeWidth="1.5" strokeLinecap="round"/>
    <line x1="7" y1="5" x2="17" y2="5" stroke="#c9a227" strokeWidth="1.5" strokeLinecap="round"/>
    <line x1="7" y1="19" x2="17" y2="19" stroke="#c9a227" strokeWidth="1.5" strokeLinecap="round"/>
  </svg>
);

/* ─── Eid offers ─────────────────────────────────────────────────────────── */
const offers = [
  { tag: "EID SPECIAL", title: "Celebration Food Hampers", sub: "Curated festive baskets delivered to your door", color: "#c9a227", emoji: "🎁" },
  { tag: "FREE DELIVERY", title: "Biryani & Kebab Platters", sub: "Authentic flavours for your Eid feast", color: "#e07820", emoji: "🍖" },
  { tag: "BUY 2 GET 1 FREE", title: "Dates & Sweets Collection", sub: "Premium Medjool dates & homemade sweets", color: "#c9a227", emoji: "🌙" },
  { tag: "20% OFF", title: "Family Celebration Meal Boxes", sub: "Feeds 4–8 — perfect for gathering loved ones", color: "#e07820", emoji: "🍽️" },
  { tag: "SAME-DAY DELIVERY", title: "Fresh Juices & Beverages", sub: "Cool down after a day of celebration", color: "#c9a227", emoji: "🥤" },
  { tag: "EID EXCLUSIVE", title: "Organic Fresh Produce Bundle", sub: "Farm-fresh vegetables for your Eid cooking", color: "#3a9e5c", emoji: "🥗" },
];
...
.prenav-root { --g: #1a5c1a; --gl: #2d8c2d; --orange: #e07820; --gold: #c9a227; --dark: #0d1b2a; --surf: #fff; --bg: #edf0ea; --mid: #445444; --muted: #8a9e87; font-family: 'Sora', sans-serif; }

/* Star twinkle animation */
@keyframes prenav-twinkle { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.3; transform: scale(0.6); } }
/* Lantern sway */
@keyframes prenav-sway { 0%, 100% { transform: rotate(-6deg); } 50% { transform: rotate(6deg); } }
/* Moon float */
...
@keyframes prenav-fadeUp { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }

/* Eid gradient — deep teal to night blue to forest green */
.prenav-wb { position: relative; overflow: hidden; background: linear-gradient(125deg, #0d1b2a 0%, #0d2b1e 30%, #1a4a2a 58%, #1a3a0e 82%, #2a5c1a 100%); }
.prenav-wb-grain { position: absolute; inset: 0; pointer-events: none; z-index: 1; opacity: 0.22; background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.88' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.045'/%3E%3C/svg%3E"); }
.prenav-wb-sweep { position: absolute; inset: 0; z-index: 2; pointer-events: none; background: linear-gradient(105deg, transparent 35%, rgba(201,162,39,.08) 50%, transparent 65%); background-size: 300% 100%; animation: prenav-shimmer 6s infinite; }

/* Floating stars */
.prenav-star { position: absolute; pointer-events: none; z-index: 1; }
.prenav-star-1 { top: 8px; left: 12%; animation: prenav-twinkle 2.1s infinite; }
.prenav-star-2 { top: 14px; left: 28%; animation: prenav-twinkle 3.0s infinite 0.4s; }
.prenav-star-3 { top: 6px; left: 50%; animation: prenav-twinkle 2.5s infinite 0.8s; }
.prenav-star-4 { top: 18px; left: 70%; animation: prenav-twinkle 1.8s infinite 1.1s; }
.prenav-star-5 { top: 5px; right: 18%; animation: prenav-twinkle 2.8s infinite 0.2s; }
.prenav-star-6 { top: 20px; right: 6%; animation: prenav-twinkle 2.2s infinite 0.6s; }

/* Lanterns */
.prenav-lantern { position: absolute; pointer-events: none; z-index: 1; }
.prenav-lantern-l { left: 4%; top: 2px; animation: prenav-sway 4s ease-in-out infinite; transform-origin: top center; }
.prenav-lantern-r { right: 4%; top: 2px; animation: prenav-sway 4.5s ease-in-out infinite 0.5s; transform-origin: top center; }
...
.prenav-pill { display: inline-flex; align-items: center; gap: 4px; background: rgba(201,162,39,.25); border: 1px solid rgba(201,162,39,.45); backdrop-filter: blur(6px); border-radius: 100px; padding: 2px 9px; font-size: 8.5px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.8px; color: #f0c060; width: fit-content; }
.prenav-pill span { animation: prenav-sparkle 2.5s infinite; display: inline-block; }
.prenav-otag { display: inline-block; font-size: 8px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.8px; color: #f0c060; background: rgba(201,162,39,.2); border-radius: 4px; padding: 1px 6px; margin-bottom: 2px; border: 1px solid rgba(201,162,39,.3); }
...
.prenav-dot.on { width: 16px; border-radius: 2px; background: rgba(201,162,39,.9); }

.prenav-shop-btn { background: linear-gradient(135deg, #c9a227, #e07820); color: #fff; border: none; border-radius: 100px; font-family: 'Sora', sans-serif; font-weight: 800; cursor: pointer; display: flex; align-items: center; gap: 4px; box-shadow: 0 3px 14px rgba(201,162,39,.35); transition: transform .15s, box-shadow .2s; flex-shrink: 0; white-space: nowrap; text-decoration: none; }
.prenav-shop-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(201,162,39,.45); }
...
.prenav-mob-title i { color: #f0c060; }
...
.prenav-mob-r2 { display: flex; align-items: center; gap: 9px; background: rgba(255,255,255,.08); border: 1px solid rgba(201,162,39,.25); backdrop-filter: blur(8px); border-radius: 12px; padding: 9px 10px; animation: prenav-offerIn .35s ease; text-decoration: none; }
...
.prenav-desk-title i { color: #f0c060; }
...
/* Arabic-style geometric accent line */
.prenav-geom { display: flex; align-items: center; gap: 6px; }
.prenav-geom-line { flex: 1; height: 1px; background: linear-gradient(to right, transparent, rgba(201,162,39,.5), transparent); }
.prenav-geom-diamond { width: 6px; height: 6px; background: #c9a227; transform: rotate(45deg); flex-shrink: 0; }

.prenav-ann { background: #0a1508; height: 32px; overflow: hidden; position: relative; }
...
.prenav-ann-dot { width: 3px; height: 3px; border-radius: 50%; background: #c9a227; flex-shrink: 0; }
.prenav-ann::before, .prenav-ann::after { content: ''; position: absolute; top: 0; bottom: 0; width: 36px; z-index: 2; pointer-events: none; }
.prenav-ann::before { left: 0; background: linear-gradient(to right, #0a1508, transparent); }
.prenav-ann::after { right: 0; background: linear-gradient(to left, #0a1508, transparent); }
    ...
    <div className="prenav-root">
      ...
      {show && (
        <div className="prenav-wb">
          ...
          {/* Animated stars scattered across banner */}
          {stars.map((st, i) => (
            <div
              key={i}
              style={{
                ...
                zIndex: 1,
                pointerEvents: "none",
                animation: `prenav-twinkle ${st.dur}s infinite ${st.delay}s`,
              }}
            >
              <StarFill s={st.s} opacity={0.55 + Math.random() * 0.45} />
            </div>
          ))}

          {/* Lanterns — desktop only via CSS */}
          <div className="prenav-lantern prenav-lantern-l"><LanternIcon s={28} /></div>
          <div className="prenav-lantern prenav-lantern-r"><LanternIcon s={28} /></div>
          ...
          <div className="prenav-wb-mob">
            <div className="prenav-mob-r1">
              <div className="prenav-mob-brand">
                <div className="prenav-mob-emoji"><CrescentMoon s={26} /></div>
                <div className="prenav-mob-text">
                  <div className="prenav-pill"><span><StarFill s={8} /></span>Eid Mubarak</div>
                  <div className="prenav-mob-title">Celebrate <i>Eid</i> with us</div>
                </div>
              </div>
              <div className="prenav-mob-cd">
                <div className="prenav-mob-cd-label">Eid al-Fitr 2026</div>
          ...
          <div className="prenav-wb-desk">
            <div className="prenav-desk-left">
              <div className="prenav-desk-emoji"><CrescentMoon s={38} /></div>
              <div className="prenav-desk-badge">
                <div className="prenav-pill"><span><StarFill s={9} /></span>Eid Mubarak</div>
                <div className="prenav-desk-title">Celebrate <i>Eid</i> with YooKatale</div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
              <StarFill s={12} opacity={0.7} />
              <span style={{ fontSize: 13, fontWeight: 800, color: "#f0c060", letterSpacing: 0.5 }}>Eid al-Fitr 2026</span>
              <StarFill s={12} opacity={0.7} />
            </div>
          </div>
        </div>
        ...
        <div className="prenav-ann-track">
          {[0, 1].map((loop) => (
            <span key={loop} style={{ display: "inline-flex", alignItems: "center" }}>
              <span className="prenav-ann-item" style={{ color: "#f0c060" }}>🌙 Eid Mubarak from YooKatale!</span><span className="prenav-ann-dot" />
              ...
              <Link href="/marketplace" className="prenav-ann-item prenav-ann-link" style={{ color: "#f0c060" }}>Eid Special Offers — Celebrate with great food</Link><span className="prenav-ann-dot" />
              <span className="prenav-ann-item">Same-day delivery across Kampala</span><span className="prenav-ann-dot" />
              <span className="prenav-ann-item" style={{ color: "#f0c060" }}>Earn loyalty points on every Eid order ⭐</span><span className="prenav-ann-dot" />
            </span>
          ))}
        </div>
      </div>
